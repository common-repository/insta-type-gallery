...
<style>
	...
	<!-- new css-->
	.loading {
		background: transparent url('<?php echo esc_url(plugin_dir_url( __FILE__ ). 'img/loading-image.gif' );?>') center no-repeat;
	}

	//pagination
	.holder-<?php echo esc_attr( $insta_gallery_id ); ?> {
		margin: 15px 0;
	}

	.holder-<?php echo esc_attr( $insta_gallery_id ); ?> a {
		font-size: 18px;
		font-weight: bolder;
		cursor: pointer;
		margin: 0 5px;
	...
	.holder-<?php echo esc_attr( $insta_gallery_id ); ?> a:hover {
		color: #FFF !important;
	...
	.holder-<?php echo esc_attr( $insta_gallery_id ); ?> a.jp-previous { 
		margin-right: 15px;
	...
	.holder-<?php echo esc_attr( $insta_gallery_id ); ?> a.jp-next { 
		margin-left: 15px;
		color: <?php echo esc_html( $button_color ); ?> !important;
		background-color: <?php echo esc_html( $button_bg_color ); ?> !important;
		padding: 10px !important;
	...
	.holder-<?php echo esc_attr( $insta_gallery_id ); ?> a.jp-current, a.jp-current:hover {
		font-weight: bold;
		color: <?php echo esc_html( $sp_color ); ?>;
		background-color: <?php echo esc_html( $spbg_color ); ?>;
		padding: 10px !important;
		text-decoration: none !important;
	}

	.holder-<?php echo esc_attr( $insta_gallery_id ); ?> a.jp-disabled, a.jp-disabled:hover {
	...
	.holder-<?php echo esc_attr( $insta_gallery_id ); ?> span { 
		font-size: 18px;
		font-weight: bolder;
		margin: 0 5px; 
	}
	
	#insta_gallery_<?php echo esc_attr( $insta_gallery_id ); ?> {
		margin-top: 20px;
	}
	...
	<div class="row spacing">
		...
		<?php if ( $upload_image == '' ) { ?>
		<img src="<?php echo esc_url(plugin_dir_url( __FILE__ ). 'img/instagram-type-gallery-premium.png' );?>" class="img-circle">
		<?php } else { ?>
			<img src="<?php echo esc_url( $upload_image ); ?>" class="img-circle">
			<?php } ?>
		</div>
		<div class="col-md-8 col-sm-7 col-xs-12 itg-spacing">
			<div class="col-md-6 col-sm-6 col-xs-12 itg-spacing"><h1 class="custom-title"><?php echo esc_html( $pro_title ); ?></h1></div>
			<div class="col-md-6 col-sm-6 col-xs-12"><div class="follow-btn"><a href="https://www.instagram.com/<?php echo esc_html( $insta_user ); ?>/" target="_blank" class="btn btn-info custom-btn"><i class="fa fa-instagram"></i> <?php echo esc_html( $follow_btn_text ); ?></a></div></div><br><br>
			
			<div class="col-md-12 col-sm-12 col-xs-12 itg-spacing">
				<div class="custome-div">
					<p class="text-justify"><?php echo esc_html( $pro_dec ); ?></p>
				</div>
			</div>
			
			<div class="col-md-12 col-sm-12 col-xs-12 itg-spacing">
				<div class="custome-div">
					<span class="counts ifg-stats"><b><a href="" class=""><?php echo esc_html( $num_post ); ?></a></b> Posts</span>
					<span class="counts ifg-stats"><b><a href=""><?php echo esc_html( $num_folo ); ?></a></b> Followers</span>
					<span class="counts ifg-stats"><b><a href=""><?php echo esc_html( $num_of_folo ); ?></a></b> Following</span>
				</div>
			</div>
		</div>
	</div>
	...
	<!-- Insta Gallery --->
	<div id="insta_gallery_<?php echo esc_attr( $insta_gallery_id ); ?>" class="row all-images">
		<?php
		...
		if ( isset( $insta_setting['slide-ids'] ) && count( $insta_setting['slide-ids'] ) > 0 ) {
			...
			foreach ( $insta_setting['slide-ids'] as $attachment_id ) {
				...
				?>
					<div class="single-image <?php echo esc_attr( $col_large_desktops ); ?> <?php echo esc_attr( $col_desktops ); ?> <?php echo esc_attr( $col_tablets ); ?> <?php echo esc_attr( $col_phones ); ?>">
						<a href="<?php echo esc_url( $full[0] ); ?>" data-toggle="lightbox" data-gallery="multiimages" data-title="<?php echo esc_html( $title ); ?>">
						
						<img class="thumbnail loading thumbnail-border animated <?php echo esc_attr( $animation_effect ); ?> holder-<?php echo esc_attr( $insta_gallery_id ); ?> <?php echo esc_attr( $image_hover_effect ); ?>" src="<?php echo esc_url( $thumbnail_url ); ?>" alt="<?php echo esc_html( $title ); ?>">				
			...
			esc_html_e( 'Sorry! No image gallery found', 'insta-type-gallery' );
		} // end of if esle of slides avaialble check into slider
		?>
	</div>
	<!-- navigation holder -->
	<div class='holder-<?php echo esc_attr( $insta_gallery_id ); ?> text-center'></div>
