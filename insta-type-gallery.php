...
if ( ! class_exists( 'Instagram_Type_Gallery' ) ) {
	...
	class Instagram_Type_Gallery {
		...
		public function set_insta_type_gallery_shortcode_column_name( $defaults ) {
			...
			foreach ( $defaults as $key => $value ) {
				if ( $key == 'date' ) {  // when we find the date column
					$new['insta_type_gallery_shortcode'] = __( 'Shortcode', 'insta-type-gallery' );  // put the tags column before it
				}
				$new[ $key ] = $value;
			}
			return $new;
		...
		public function custom_insta_type_gallery_shodrcode_data( $column, $post_id ) {
			switch ( $column ) {
				case 'insta_type_gallery_shortcode':
					echo "<input type='text' class='button button-primary' id='insta-type-shortcode-" . esc_attr( $post_id ) . "' value='[ITG id=" . esc_attr( $post_id ) . "]' style='font-weight:bold; background-color:#32373C; color:#FFFFFF; text-align:center;' />";
					echo "<input type='button' class='button button-primary' onclick='return INSTACopyShortcode" . esc_attr( $post_id ) . "();' readonly value='Copy' style='margin-left:4px;' />";
					...
					echo '<script>
						function INSTACopyShortcode' . esc_attr( $post_id ) . "() {
							var copyText = document.getElementById('insta-type-shortcode-" . esc_attr( $post_id ) . "');
							copyText.select();
		...
		public function _load_textdomain() {
			load_plugin_textdomain( 'insta-type-gallery', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
		}
		...
		public function _insta_menu() {
			$help_menu = add_submenu_page( 'edit.php?post_type=' . ITG_PLUGIN_SLUG, __( 'Docs', 'insta-type-gallery' ), __( 'Docs', 'insta-type-gallery' ), 'administrator', 'sr-doc-page', array( $this, '_itg_doc_page' ) );
			$our_theme = add_submenu_page( 'edit.php?post_type=' . ITG_PLUGIN_SLUG, __( 'Our Theme', 'insta-type-gallery' ), __( 'Our Theme', 'insta-type-gallery' ), 'administrator', 'sr-theme-page', array( $this, '_itg_theme_page' ) );
		}
		...
		public function _Insta_Gallery() {
			$labels = array(
				'name'               => _x( 'Insta Type Gallery', 'Post Type General Name', 'insta-type-gallery' ),
				'singular_name'      => _x( 'Insta Type Gallery', 'Post Type Singular Name', 'insta-type-gallery' ),
				'menu_name'          => __( 'Insta Type Gallery', 'insta-type-gallery' ),
				'name_admin_bar'     => __( 'Insta Type Gallery', 'insta-type-gallery' ),
				'parent_item_colon'  => __( 'Parent Item:', 'insta-type-gallery' ),
				'all_items'          => __( 'All Insta Gallery', 'insta-type-gallery' ),
				'add_new_item'       => __( 'Add New Insta Gallery', 'insta-type-gallery' ),
				'add_new'            => __( 'Add Insta Gallery', 'insta-type-gallery' ),
				'new_item'           => __( 'New Insta Gallery', 'insta-type-gallery' ),
				'edit_item'          => __( 'Edit Insta Gallery', 'insta-type-gallery' ),
				'update_item'        => __( 'Update Insta Gallery', 'insta-type-gallery' ),
				'search_items'       => __( 'Search Insta Gallery', 'insta-type-gallery' ),
				'not_found'          => __( 'Insta Gallery Not found', 'insta-type-gallery' ),
				'not_found_in_trash' => __( 'Insta Gallery Not found in Trash', 'insta-type-gallery' ),
			);
			$args   = array(
				'label'               => __( 'Insta Type GalleryInsta Type Gallery', 'insta-type-gallery' ),
				'description'         => __( 'Custom Post Type For Insta Gallery', 'insta-type-gallery' ),
				'labels'              => $labels,
		...
		public function _admin_add_meta_box() {
			...
			add_meta_box( '1', __( 'Copy Album Gallery Shortcode', 'insta-type-gallery' ), array( &$this, '_itg_shortcode_left_metabox' ), 'insta_type_gallery', 'side', 'default' );
			add_meta_box( __( 'Add Image', 'insta-type-gallery' ), __( 'Add Image', 'insta-type-gallery' ), array( &$this, 'itg_upload_multiple_images' ), 'insta_type_gallery', 'normal', 'default' );
			add_meta_box( __( 'Rate Our Plugin', 'insta-type-gallery' ), __( 'Rate Our Plugin', 'insta-type-gallery' ), array( &$this, 'itg_rate_plugin' ), 'insta_type_gallery', 'side', 'default' );
		}
		// Insta Type gallery copy shortcode meta box under publish button
		public function _itg_shortcode_left_metabox( $post ) { ?>
			<p class="input-text-wrap">
				<input type="text" name="insta-type-shortcode" id="insta-type-shortcode" value="<?php echo '[ITG id=' . esc_attr( $post->ID ) . ']'; ?>" readonly style="height: 60px; text-align: center; width:100%;  font-size: 24px; border: 2px dashed;">
				<p id="itg-copy-code"><?php esc_html_e( 'Shortcode copied to clipboard!', 'insta-type-gallery' ); ?></p>
				<p style="margin-top: 10px"><?php esc_html_e( 'Copy & Embed shotcode into any Page/ Post to display gallery.', 'insta-type-gallery' ); ?></p>
			</p>
		...
		<div style="text-align:center">
			<p><?php esc_html_e( 'If you like our plugin then please', 'insta-type-gallery' ); ?> <b><?php esc_html_e( 'Rate us', 'insta-type-gallery' ); ?></b> <?php esc_html_e( 'on WordPress', 'insta-type-gallery' ); ?></p>
		</div>
		<div style="text-align:center">
			...
			<div class="row">
				...
				<div class="file-upload">
					<div class="image-upload-wrap">
						...
						<div class="drag-text">
							<h3><?php esc_html_e( 'ADD IMAGES', 'insta-type-gallery' ); ?></h3>
						</div>
					</div>
				</div>
			</div>
		...
		public function _ajax_insta_gallery() {
			echo esc_attr( $this->_itg_ajax_callback_function( $_POST['slideId'] ) );
			die;
		}
	...
	// register sf scripts
	function awplife_itg_register_scripts() {

		// css & JS
		wp_register_script( 'itg-bootstrap-js', plugin_dir_url( __FILE__ ) . 'assets/js/bootstrap.min.js', array( 'jquery' ), '', true );
		wp_register_script( 'itg-jpages-js', plugin_dir_url( __FILE__ ) . 'assets/js/itg-jPages.js' );
		wp_register_script( 'itg-imagesloaded-pkgd-js', plugin_dir_url( __FILE__ ) . 'assets/js/imagesloaded.pkgd.js', array( 'jquery' ), '', true );
		wp_register_script( 'itg-isotope-js', plugin_dir_url( __FILE__ ) . 'assets/js/isotope.pkgd.js', array( 'jquery' ), '', false );

		wp_register_style( 'itg-bootstrap-css', plugin_dir_url( __FILE__ ) . 'assets/css/bootstrap.css' );
		wp_register_style( 'itg-jpages-css', plugin_dir_url( __FILE__ ) . 'assets/css/jPages.css' );
		wp_register_style( 'itg-animate-css', plugin_dir_url( __FILE__ ) . 'assets/css/animate.css' );
		wp_register_style( 'itg-font-awesome-css', plugin_dir_url( __FILE__ ) . 'assets/css/font-awesome.css' );
		// css & JS
	}
		add_action( 'wp_enqueue_scripts', 'awplife_itg_register_scripts' );

	// Plugin Recommend
		add_action( 'tgmpa_register', 'ITG_TXTDM_plugin_recommend' );
	function ITG_TXTDM_plugin_recommend() {
		$plugins = array(
			array(
				'name'     => 'Pricing Table',
